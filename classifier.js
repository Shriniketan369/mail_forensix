...
async function classifyEmail(parsedEmail) {
  ...
  const reasoning = buildReasoning(verdict, triggered, parsedEmail);
...
function buildReasoning(verdict, triggered, parsed) {
  if (triggered.length === 0) {
    return 'No phishing indicators were detected. Authentication checks passed and the message content shows no urgency, threat, or credential-harvesting patterns.';
  }
  ...
  return `${verdictPhrase}: ${topReasons.join('; ')}${triggered.length > 3 ? `, and ${triggered.length - 3} more` : ''}.`;
}
